if ( TRACE ) { TRACE( JSON.parse( '["BagSlot#Busy#get","BagSlot#IsFree#get","BagSlot#Data#get","BagSlot#Position#get","BagSlot#Booked#get","BagSlot#Awake","BagSlot#Book","BagSlot#StopBooking","BagSlot#Empty","BagSlot#Put","GameComponent#Init","DeckLayer#AnyFreeSlot#get","DeckLayer#Init","DeckLayer#InitSlots","DeckLayer#FindSlots","DeckLayer#GetFreeSlot","Game#Tiles#get","Game#LevelData#get","Game#Bag#get","Game#MatchCondition#get","Game#init","Game#Start","Game#InitComponents","Game#Spawn","Game#Spawned","Game#OnValidate","Tile#Position#get","Tile#init","Tile#SetContacts","Tile#SetSlot","Tile#Enable","Tile#Disable","Tile#ResetPosition","Tile#OnMouseDown","Tile#Set","TileData#CanSpawn#get","TileData#Decrease","TilesBag#BusySlots#get","TilesBag#HaveEmptySlot#get","TilesBag#EmptySlot#get","TilesBag#NoFreeSpace#get","TilesBag#init","TilesBag#Put","TilesBag#Remove","TileSlot#IsFree#get","TileSlot#Data#get","TileSlot#Position#get","TileSlot#init","TileSlot#MoveToPosition","TileSlot#Init","TileSlot#Put","TileSlot#Update","TileSlot#SpawnAnimation","TileSlot#PutInBag","TileSlot#Empty","TileSlot#Hide","TileSlot#Show","Deck#Slots#get","Deck#FirstLayer#get","Deck#SecondLayer#get","Deck#OnInit","Deck#FindPlatforms","Deck#InitPlatforms","LoseCondition#init","LoseCondition#OnInit","LoseCondition#OnBagChange","LoseCondition#Update","LoseCondition#Check","LoseCondition#Lose","MatchCondition#Completed#get","MatchCondition#init","MatchCondition#OnInit","MatchCondition#Check","MatchCondition#Match","TileActions#OnInit","TileActions#Observe","TileActions#Click","TileActions#PutBag","TileLocker#OnInit","TileLocker#Refresh","TileSpawnAnimation#init","TileSpawnAnimation#SpawnAnimation","TileSpawnAnimation#Play","TileSpawnAnimation#OnInit","TileSpawner#Level#get","TileSpawner#init","TileSpawner#OnInit","TileSpawner#Spawn","TileSpawner#RunSpawn","TileSpawner#GetRandom$1","TileSpawner#GetRandom","WinCondition#Completed#get","WinCondition#init","WinCondition#OnInit","WinCondition#Check"]' ) ); }
/**
...
if ( TRACE ) { TRACE( "DeckLayer#FindSlots", this ); }
                ...
                UnityEngine.Debug.Log$1((Bridge.ensureBaseProperty(this, "name").$UnityEngine$Component$name || "") + ": slots total " + this.Slots.length);
            },
...
if ( TRACE ) { TRACE( "TilesBag#EmptySlot#get", this ); }
            ...
            NoFreeSpace: {
                get: function () {
if ( TRACE ) { TRACE( "TilesBag#NoFreeSpace#get", this ); }
...
if ( TRACE ) { TRACE( "TileSlot#init", this ); }
        ...
        methods: {
            /*TileSlot.MoveToPosition start.*/
            MoveToPosition: function (slot) {
if ( TRACE ) { TRACE( "TileSlot#MoveToPosition", this ); }
...
if ( TRACE ) { TRACE( "Deck#FindPlatforms", this ); }
                ...
                UnityEngine.Debug.Log$1("Layers total: " + this.Layers.length);
            },
...
if ( TRACE ) { TRACE( "MatchCondition#Check", this ); }
                ...
                $t = Bridge.getEnumerator(bag.slots);
                try {
                    while ($t.moveNext()) {
                        ...
                        var amount = System.Linq.Enumerable.from(bag.BusySlots, BagSlot).count((function ($me, slot) {
                                return function (check) {
                                    return check.Data.type === slot.v.Data.type;
                                };
                            })(this, slot));
                        if (amount !== 3) {
                            continue;
...
if ( TRACE ) { TRACE( "TileActions#Observe", this ); }
                ...
                try {
                    while ($t.moveNext()) {
                        var tile = $t.Current;
                        tile.addOnClick(Bridge.fn.cacheBind(this, this.Click));
                    }
                } finally {
                    if (Bridge.is($t, System.IDisposable)) {
                        $t.System$IDisposable$Dispose();
...
if ( TRACE ) { TRACE( "TileActions#Click", this ); }
                ...
                UnityEngine.Debug.Log$1((tile.icon.sprite.name || "") + " clicked");
                if (!tile.Slot.InBag) {
                    var bag = this.Game.Bag;
...
if ( TRACE ) { TRACE( "TileLocker#Refresh", this ); }
                ...
                var $t;
                UnityEngine.Debug.LogWarning$1("TILES" + tiles.Count);
                for (var i = 0; i < ((layers.length - 1) | 0); i = (i + 1) | 0) {
                    var layer = layers[i];
                    var current = { v : layer.id };
                    var onTop = { v : layers[((i + 1) | 0)].id };
                    UnityEngine.Debug.LogWarning$1("Current: " + current.v + ", onTop:" + onTop.v);
                    ...
                    try {
                        while ($t.moveNext()) {
                            ...
                            UnityEngine.Debug.Log$1("Top tiles: " + topTiles.Count);
                            ...
                            UnityEngine.Debug.LogWarning$1("coverTiles tiles: " + coverTiles.Count);
                    ...
                    try {
                        for (;;) {
                            switch ($step) {
                                ...
                                case 2: {
                                    $t1 = Bridge.getEnumerator(group3);
                                        $step = 3;
                                        continue;
                                }
                                case 3: {
                                ...
                                case 6: {
                                    UnityEngine.Debug.Log$1("New group with Y value: " + group3.getItem(0).Layer + " and count: " + group3.Count);
                                ...
                                case 11: {
                                    $t3 = Bridge.getEnumerator(group2);
                                        $step = 12;
                                        continue;
                                }
                                case 12: {
                                ...
                                case 15: {
                                    UnityEngine.Debug.Log$1("New group with Y value: " + group2.getItem(0).Layer + " and count: " + group2.Count);
                                        $enumerator.current = new UnityEngine.WaitForSeconds(this.delayBetweenLines);
                                        $step = 16;
                                        return true;
                                }
...
if ( TRACE ) { TRACE( "WinCondition#Check", this ); }
    ...
    /*MatchCondition start.*/
    $m("MatchCondition", function () { return {"att":1048577,"a":2,"m":[{"a":2,"isSynthetic":true,"n":".ctor","t":1,"sn":"ctor"},{"a":1,"n":"Check","t":8,"sn":"Check","rt":$n[0].Void},{"a":1,"n":"Match","t":8,"pi":[{"n":"bagSlots","pt":$n[2].List$1(BagSlot),"ps":0}],"sn":"Match","rt":$n[0].Void,"p":[$n[2].List$1(BagSlot)]},{"ov":true,"a":3,"n":"OnInit","t":8,"sn":"OnInit","rt":$n[0].Void},{"a":2,"n":"Completed","t":16,"rt":$n[0].Int32,"g":{"a":2,"n":"get_Completed","t":8,"rt":$n[0].Int32,"fg":"Completed","box":function ($v) { return Bridge.box($v, System.Int32);}},"fn":"Completed"},{"a":2,"n":"completeSlots","t":4,"rt":$n[2].List$1(TileSlot),"sn":"completeSlots"},{"a":2,"n":"OnMatch","t":2,"ad":{"a":2,"n":"add_OnMatch","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"addOnMatch","rt":$n[0].Void,"p":[Function]},"r":{"a":2,"n":"remove_OnMatch","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"removeOnMatch","rt":$n[0].Void,"p":[Function]}}]}; }, $n);
    /*MatchCondition end.*/

    /*Tile start.*/
    $m("Tile", function () { return {"att":1048577,"a":2,"m":[{"a":2,"isSynthetic":true,"n":".ctor","t":1,"sn":"ctor"},{"a":2,"n":"Disable","t":8,"sn":"Disable","rt":$n[0].Void},{"a":2,"n":"Enable","t":8,"sn":"Enable","rt":$n[0].Void},{"a":1,"n":"OnMouseDown","t":8,"sn":"OnMouseDown","rt":$n[0].Void},{"a":1,"n":"ResetPosition","t":8,"sn":"ResetPosition","rt":$n[0].Void},{"a":2,"n":"Set","t":8,"pi":[{"n":"data","pt":TileData,"ps":0},{"n":"layer","pt":$n[0].Int32,"ps":1}],"sn":"Set","rt":$n[0].Void,"p":[TileData,$n[0].Int32]},{"a":2,"n":"SetContacts","t":8,"pi":[{"n":"cover","pt":$n[2].List$1(Tile),"ps":0}],"sn":"SetContacts","rt":$n[0].Void,"p":[$n[2].List$1(Tile)]},{"a":2,"n":"SetSlot","t":8,"pi":[{"n":"slot","pt":TileSlot,"ps":0}],"sn":"SetSlot","rt":$n[0].Void,"p":[TileSlot]},{"a":2,"n":"Data","t":16,"rt":TileData,"g":{"a":2,"n":"get_Data","t":8,"rt":TileData,"fg":"Data"},"s":{"a":1,"n":"set_Data","t":8,"p":[TileData],"rt":$n[0].Void,"fs":"Data"},"fn":"Data"},{"a":1,"n":"IsClickable","t":16,"rt":$n[0].Boolean,"g":{"a":1,"n":"get_IsClickable","t":8,"rt":$n[0].Boolean,"fg":"IsClickable","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},"s":{"a":1,"n":"set_IsClickable","t":8,"p":[$n[0].Boolean],"rt":$n[0].Void,"fs":"IsClickable"},"fn":"IsClickable"},{"a":2,"n":"Layer","t":16,"rt":$n[0].Int32,"g":{"a":2,"n":"get_Layer","t":8,"rt":$n[0].Int32,"fg":"Layer","box":function ($v) { return Bridge.box($v, System.Int32);}},"s":{"a":1,"n":"set_Layer","t":8,"p":[$n[0].Int32],"rt":$n[0].Void,"fs":"Layer"},"fn":"Layer"},{"a":2,"n":"Position","t":16,"rt":$n[1].Vector2,"g":{"a":2,"n":"get_Position","t":8,"rt":$n[1].Vector2,"fg":"Position"},"fn":"Position"},{"a":2,"n":"Slot","t":16,"rt":TileSlot,"g":{"a":2,"n":"get_Slot","t":8,"rt":TileSlot,"fg":"Slot"},"s":{"a":1,"n":"set_Slot","t":8,"p":[TileSlot],"rt":$n[0].Void,"fs":"Slot"},"fn":"Slot"},{"at":[new UnityEngine.Serialization.FormerlySerializedAsAttribute("contacters")],"a":2,"n":"coverTiles","t":4,"rt":$n[2].List$1(Tile),"sn":"coverTiles"},{"a":2,"n":"disabledTile","t":4,"rt":$n[1].SpriteRenderer,"sn":"disabledTile"},{"a":2,"n":"icon","t":4,"rt":$n[1].SpriteRenderer,"sn":"icon"},{"a":2,"n":"OnClick","t":2,"ad":{"a":2,"n":"add_OnClick","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"addOnClick","rt":$n[0].Void,"p":[Function]},"r":{"a":2,"n":"remove_OnClick","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"removeOnClick","rt":$n[0].Void,"p":[Function]}},{"a":1,"backing":true,"n":"<Data>k__BackingField","t":4,"rt":TileData,"sn":"Data"},{"a":1,"backing":true,"n":"<IsClickable>k__BackingField","t":4,"rt":$n[0].Boolean,"sn":"IsClickable","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},{"a":1,"backing":true,"n":"<Layer>k__BackingField","t":4,"rt":$n[0].Int32,"sn":"Layer","box":function ($v) { return Bridge.box($v, System.Int32);}},{"a":1,"backing":true,"n":"<Slot>k__BackingField","t":4,"rt":TileSlot,"sn":"Slot"}]}; }, $n);
    /*Tile end.*/

    /*TileActions start.*/
    $m("TileActions", function () { return {"att":1048577,"a":2,"m":[{"a":2,"isSynthetic":true,"n":".ctor","t":1,"sn":"ctor"},{"a":1,"n":"Click","t":8,"pi":[{"n":"tile","pt":Tile,"ps":0}],"sn":"Click","rt":$n[0].Void,"p":[Tile]},{"a":2,"n":"Observe","t":8,"pi":[{"n":"tiles","pt":$n[2].List$1(Tile),"ps":0}],"sn":"Observe","rt":$n[0].Void,"p":[$n[2].List$1(Tile)]},{"ov":true,"a":3,"n":"OnInit","t":8,"sn":"OnInit","rt":$n[0].Void},{"a":1,"n":"PutBag","t":8,"pi":[{"n":"moving","pt":TileSlot,"ps":0},{"n":"bagSlot","pt":BagSlot,"ps":1}],"sn":"PutBag","rt":$n[0].Void,"p":[TileSlot,BagSlot]}]}; }, $n);
    /*TileActions end.*/

    /*TileData start.*/
    $m("TileData", function () { return {"att":1056769,"a":2,"at":[new System.SerializableAttribute()],"m":[{"a":2,"isSynthetic":true,"n":".ctor","t":1,"sn":"ctor"},{"a":2,"n":"Decrease","t":8,"sn":"Decrease","rt":$n[0].Void},{"a":2,"n":"CanSpawn","t":16,"rt":$n[0].Boolean,"g":{"a":2,"n":"get_CanSpawn","t":8,"rt":$n[0].Boolean,"fg":"CanSpawn","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},"fn":"CanSpawn"},{"a":2,"n":"amount","t":4,"rt":$n[0].Int32,"sn":"amount","box":function ($v) { return Bridge.box($v, System.Int32);}},{"a":2,"n":"icon","t":4,"rt":$n[1].Sprite,"sn":"icon"},{"at":[new UnityEngine.Serialization.FormerlySerializedAsAttribute("id")],"a":2,"n":"type","t":4,"rt":$n[0].Int32,"sn":"type","box":function ($v) { return Bridge.box($v, System.Int32);}}]}; }, $n);
    /*TileData end.*/
    ...
    /*TilesBag start.*/
    $m("TilesBag", function () { return {"att":1048577,"a":2,"m":[{"a":2,"isSynthetic":true,"n":".ctor","t":1,"sn":"ctor"},{"a":2,"n":"Put","t":8,"pi":[{"n":"tileSlot","pt":TileSlot,"ps":0},{"n":"bagSlot","pt":BagSlot,"ps":1}],"sn":"Put","rt":$n[0].Void,"p":[TileSlot,BagSlot]},{"a":2,"n":"Remove","t":8,"pi":[{"n":"slot","pt":TileSlot,"ps":0}],"sn":"Remove","rt":$n[0].Void,"p":[TileSlot]},{"a":2,"n":"BusySlots","t":16,"rt":$n[2].List$1(BagSlot),"g":{"a":2,"n":"get_BusySlots","t":8,"rt":$n[2].List$1(BagSlot),"fg":"BusySlots"},"fn":"BusySlots"},{"a":2,"n":"EmptySlot","t":16,"rt":BagSlot,"g":{"a":2,"n":"get_EmptySlot","t":8,"rt":BagSlot,"fg":"EmptySlot"},"fn":"EmptySlot"},{"a":2,"n":"HaveEmptySlot","t":16,"rt":$n[0].Boolean,"g":{"a":2,"n":"get_HaveEmptySlot","t":8,"rt":$n[0].Boolean,"fg":"HaveEmptySlot","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},"fn":"HaveEmptySlot"},{"a":2,"n":"NoFreeSpace","t":16,"rt":$n[0].Boolean,"g":{"a":2,"n":"get_NoFreeSpace","t":8,"rt":$n[0].Boolean,"fg":"NoFreeSpace","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},"fn":"NoFreeSpace"},{"a":2,"n":"slots","t":4,"rt":$n[2].List$1(BagSlot),"sn":"slots"},{"a":2,"n":"OnPut","t":2,"ad":{"a":2,"n":"add_OnPut","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"addOnPut","rt":$n[0].Void,"p":[Function]},"r":{"a":2,"n":"remove_OnPut","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"removeOnPut","rt":$n[0].Void,"p":[Function]}}]}; }, $n);
    /*TilesBag end.*/

    /*TileSlot start.*/
    $m("TileSlot", function () { return {"att":1048577,"a":2,"m":[{"a":2,"isSynthetic":true,"n":".ctor","t":1,"sn":"ctor"},{"a":2,"n":"Empty","t":8,"sn":"Empty","rt":$n[0].Void},{"a":2,"n":"Hide","t":8,"sn":"Hide","rt":$n[0].Void},{"a":2,"n":"Init","t":8,"pi":[{"n":"id","pt":$n[0].Int32,"ps":0}],"sn":"Init","rt":$n[0].Void,"p":[$n[0].Int32]},{"a":2,"n":"MoveToPosition","t":8,"pi":[{"n":"slot","pt":BagSlot,"ps":0}],"sn":"MoveToPosition","rt":$n[0].Void,"p":[BagSlot]},{"a":2,"n":"Put","t":8,"pi":[{"n":"put","pt":Tile,"ps":0}],"sn":"Put","rt":$n[0].Void,"p":[Tile]},{"a":2,"n":"PutInBag","t":8,"sn":"PutInBag","rt":$n[0].Void},{"a":2,"n":"Show","t":8,"sn":"Show","rt":$n[0].Void},{"a":2,"n":"SpawnAnimation","t":8,"sn":"SpawnAnimation","rt":$n[0].Void},{"a":1,"n":"Update","t":8,"sn":"Update","rt":$n[0].Void},{"a":2,"n":"Data","t":16,"rt":TileData,"g":{"a":2,"n":"get_Data","t":8,"rt":TileData,"fg":"Data"},"fn":"Data"},{"a":2,"n":"InBag","t":16,"rt":$n[0].Boolean,"g":{"a":2,"n":"get_InBag","t":8,"rt":$n[0].Boolean,"fg":"InBag","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},"s":{"a":1,"n":"set_InBag","t":8,"p":[$n[0].Boolean],"rt":$n[0].Void,"fs":"InBag"},"fn":"InBag"},{"a":2,"n":"IsFree","t":16,"rt":$n[0].Boolean,"g":{"a":2,"n":"get_IsFree","t":8,"rt":$n[0].Boolean,"fg":"IsFree","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},"fn":"IsFree"},{"a":2,"n":"Layer","t":16,"rt":$n[0].Int32,"g":{"a":2,"n":"get_Layer","t":8,"rt":$n[0].Int32,"fg":"Layer","box":function ($v) { return Bridge.box($v, System.Int32);}},"s":{"a":1,"n":"set_Layer","t":8,"p":[$n[0].Int32],"rt":$n[0].Void,"fs":"Layer"},"fn":"Layer"},{"a":2,"n":"Position","t":16,"rt":$n[1].Vector2,"g":{"a":2,"n":"get_Position","t":8,"rt":$n[1].Vector2,"fg":"Position"},"fn":"Position"},{"a":2,"n":"Tile","t":16,"rt":Tile,"g":{"a":2,"n":"get_Tile","t":8,"rt":Tile,"fg":"Tile"},"s":{"a":1,"n":"set_Tile","t":8,"p":[Tile],"rt":$n[0].Void,"fs":"Tile"},"fn":"Tile"},{"a":1,"n":"_isSpawnAnimation","t":4,"rt":$n[0].Boolean,"sn":"_isSpawnAnimation","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},{"a":1,"n":"_spawnTimer","t":4,"rt":$n[0].Single,"sn":"_spawnTimer","box":function ($v) { return Bridge.box($v, System.Single, System.Single.format, System.Single.getHashCode);}},{"a":1,"n":"_targetPosition","t":4,"rt":$n[1].Vector3,"sn":"_targetPosition"},{"a":1,"n":"_targetSlot","t":4,"rt":BagSlot,"sn":"_targetSlot"},{"a":2,"n":"background","t":4,"rt":$n[1].SpriteRenderer,"sn":"background"},{"a":2,"n":"defaultSize","t":4,"rt":$n[1].Vector3,"sn":"defaultSize"},{"a":1,"n":"isMoving","t":4,"rt":$n[0].Boolean,"sn":"isMoving","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},{"a":2,"n":"moveDuration","t":4,"rt":$n[0].Single,"sn":"moveDuration","box":function ($v) { return Bridge.box($v, System.Single, System.Single.format, System.Single.getHashCode);}},{"a":2,"n":"spawnAnimDuration","t":4,"rt":$n[0].Single,"sn":"spawnAnimDuration","box":function ($v) { return Bridge.box($v, System.Single, System.Single.format, System.Single.getHashCode);}},{"a":2,"n":"spawnAnimSize","t":4,"rt":$n[0].Single,"sn":"spawnAnimSize","box":function ($v) { return Bridge.box($v, System.Single, System.Single.format, System.Single.getHashCode);}},{"a":2,"n":"speed","t":4,"rt":$n[0].Single,"sn":"speed","box":function ($v) { return Bridge.box($v, System.Single, System.Single.format, System.Single.getHashCode);}},{"a":1,"n":"timer","t":4,"rt":$n[0].Single,"sn":"timer","box":function ($v) { return Bridge.box($v, System.Single, System.Single.format, System.Single.getHashCode);}},{"a":2,"n":"OnMoveFinish","t":2,"ad":{"a":2,"n":"add_OnMoveFinish","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"addOnMoveFinish","rt":$n[0].Void,"p":[Function]},"r":{"a":2,"n":"remove_OnMoveFinish","t":8,"pi":[{"n":"value","pt":Function,"ps":0}],"sn":"removeOnMoveFinish","rt":$n[0].Void,"p":[Function]}},{"a":1,"backing":true,"n":"<InBag>k__BackingField","t":4,"rt":$n[0].Boolean,"sn":"InBag","box":function ($v) { return Bridge.box($v, System.Boolean, System.Boolean.toString);}},{"a":1,"backing":true,"n":"<Layer>k__BackingField","t":4,"rt":$n[0].Int32,"sn":"Layer","box":function ($v) { return Bridge.box($v, System.Int32);}},{"a":1,"backing":true,"n":"<Tile>k__BackingField","t":4,"rt":Tile,"sn":"Tile"}]}; }, $n);
    /*TileSlot end.*/
